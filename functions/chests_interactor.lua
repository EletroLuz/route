...
local MIN_MOVEMENT_DISTANCE = 0.2 -- Distancia Minima para considerar movimento
...
local POST_CHEST_PAUSE_DURATION = 10  -- Pausa de 10 segundos após abrir o baú
local last_chest_opened_time = 0      -- Nova variável para controlar o tempo
local is_paused_after_chest = false
...
local CHEST_INTERACTION_DURATION = 10  -- segundos
...
local POST_CHEST_PAUSE_DURATION = 10  -- Pausa de 3 segundos após abrir o baú

-- Modifique a função check_chest_opened
local function check_chest_opened()
    ...
    if success_by_actor or success_by_cinders then
        ...
        -- Força pausa completa
        last_chest_opened_time = os.clock()
        is_paused_after_chest = true
        ...
        console.print("=== INICIANDO PAUSA DE " .. POST_CHEST_PAUSE_DURATION .. " SEGUNDOS ===")
        return true
    end
...
local function reset_state()
    -- Não reseta se estiver em interação
    if Movement.is_interacting() then
        local current_time = os.clock()
        local interaction_end = Movement.get_interaction_end_time()
        
        if interaction_end and current_time <= interaction_end then
            -- Mantém as configurações de pausa
            Movement.set_moving(false)
            Movement.set_explorer_control(false)
            Movement.disable_anti_stuck()
            explorer.disable()
            return  -- Sai da função sem resetar o resto
        end
    end

    -- Reset de alvos e posições
    targetObject = nil
...
local stateFunctions = {
    ...
    [States.VERIFYING_CHEST_OPEN] = function()
        if not vfx_check_start_time then
            console.print("ERRO: vfx_check_start_time não definido!")
            vfx_check_start_time = os.clock()
        end
        ...
        if current_cinders < cinders_before_interaction then
            ...
            -- Define pausa após abrir o baú com sucesso
            Movement.set_interaction_end_time(os.clock() + POST_CHEST_PAUSE_DURATION)
            Movement.set_interacting(true)
            Movement.set_moving(false)
            Movement.set_explorer_control(false)
            Movement.disable_anti_stuck()
            explorer.disable()
            console.print("Iniciando pausa após abrir o baú")
        ...
        if elapsed_time > vfx_check_duration then
            ...
            if failed_attempts >= max_attempts then
                ...
                Movement.set_interacting(false)
                vfx_check_start_time = nil
                -- Define novo tempo de pausa antes de tentar novamente
                Movement.set_interaction_end_time(os.clock() + POST_CHEST_PAUSE_DURATION)
                return move_to_object(targetObject)
            end
        end
    
        -- Verifica pelo nome do ator
        if check_chest_opened() then
            console.print("Baú confirmado como aberto")
            if targetObject then
                add_to_permanent_blacklist(targetObject)
            end
...
function ChestsInteractor.interactWithObjects(doorsEnabled, interactive_patterns, current_waypoint_index)
    -- Verifica pausa após baú
    if is_paused_after_chest then
        local current_time = os.clock()
        local elapsed_time = current_time - last_chest_opened_time
        
        -- Força parada total durante a pausa
        Movement.set_moving(false)
        Movement.set_explorer_control(false)
        Movement.disable_anti_stuck()
        explorer.disable()
        
        if elapsed_time < POST_CHEST_PAUSE_DURATION then
            console.print(string.format("Pausa após baú: %.1f segundos restantes", 
                POST_CHEST_PAUSE_DURATION - elapsed_time))
            return
        else
            console.print("=== FIM DA PAUSA ===")
            is_paused_after_chest = false
            last_chest_opened_time = 0
            Movement.set_moving(true)
            Movement.enable_anti_stuck()
        end
    end
...
function ChestsInteractor.draw_chest_info()
    local y_position = 250
    local line_height = 20
    local padding = 5
    
    -- Informação básica de baús abertos
    local total_chests = ChestsInteractor.getSuccessfulChestsOpened()
    local chest_info_text = string.format("Total de Baús Helltide Abertos: %d", total_chests)
    graphics.text_2d(chest_info_text, vec2:new(10, y_position), 20, color_white)
    y_position = y_position + line_height + padding
    
    -- Status do loop inicial com progresso
    local visited_count = count_visited_waypoints()
    local loop_status
    if RouteOptimizer.has_completed_initial_loop() then
        loop_status = "Completo"
    else
        loop_status = string.format(
            "Em Progresso (%d/%d WPs)",
            visited_count,
            total_waypoints
        )
    end
    graphics.text_2d("Loop Inicial: " .. loop_status, vec2:new(10, y_position), 20, color_white)
    y_position = y_position + line_height + padding
    ...
    if RouteOptimizer.in_optimized_route and RouteOptimizer.current_route then
        ...
        graphics.text_2d(string.format(
            "Rota Otimizada: %d baús (Waypoint %d, Direção: %s)",
            #route.chests,
            route.waypoints[1],
            route.direction
        ), vec2:new(10, y_position), 20, color_green)
        y_position = y_position + line_height + padding
    end
    
    -- Título da seção de baús perdidos
    if next(missed_chests) then
        graphics.text_2d("Baús Perdidos:", vec2:new(10, y_position), 20, color_white)
        y_position = y_position + line_height + padding
        
        -- Informações de cada baú perdido
        local player_pos = get_local_player():get_position()
        local current_cinders = get_helltide_coin_cinders()
        
        for key, chest in pairs(missed_chests) do
            local distance = player_pos:dist_to(chest.position)
            local color_to_use = current_cinders >= chest.required_cinders and color_green or color_red
            
            -- Formata a string com nome, distância e custo
            local chest_text = string.format(
                "• %s (%.0fm) - Custo: %d cinders %s", 
                chest.name,
                distance,
                chest.required_cinders,
                current_cinders >= chest.required_cinders and "✓" or "✗"
            )
            
            graphics.text_2d(chest_text, vec2:new(20, y_position), 18, color_to_use)
            y_position = y_position + line_height
        end
    else
        graphics.text_2d("Nenhum baú perdido registrado", vec2:new(10, y_position), 20, color_white)
    end
    
    -- Se estiver voltando para um baú específico
    if target_missed_chest then
        y_position = y_position + line_height + padding
        local target_text = string.format(
            "Retornando para: %s (Waypoint %d)", 
            target_missed_chest.name,
            target_missed_chest.waypoint_index
        )
        graphics.text_2d(target_text, vec2:new(10, y_position), 20, color_green)
    end
    ...
    graphics.text_2d(string.format(
        "Estado Atual: %s%s",
        currentState,
        Movement.is_interacting() and " (Interagindo)" or ""
    ), vec2:new(10, y_position), 20, color_white)
    
    -- Informações de cinders
    y_position = y_position + line_height
    local current_cinders = get_helltide_coin_cinders()
    graphics.text_2d(string.format(
        "Cinders: %d",
    ...
    -- Informações de waypoint
    y_position = y_position + line_height
    local current_waypoint = Movement.get_current_waypoint_index()
    if current_waypoint then
        graphics.text_2d(string.format(
            "Waypoint Atual: %d%s",
            current_waypoint,
            Movement.is_reverse_mode() and " (Reverso)" or ""
        ), vec2:new(10, y_position), 20, color_white)
    end
end
