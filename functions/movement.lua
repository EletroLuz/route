...
local post_interaction_pause = 12  -- tempo em segundos
local post_interaction_start = nil
local INTERACTION_DURATION = 12  -- Novo: duração da interação em segundos
local POST_INTERACTION_PAUSE = 10  -- Novo: pausa após interação em segundos
...
local function handle_interacting_state(current_time)
    if interaction_end_time and current_time > interaction_end_time then
        -- Adiciona pausa extra após o término da interação
        local post_interaction_time = interaction_end_time + POST_INTERACTION_PAUSE
        
        if current_time <= post_interaction_time then
            -- Mantém o estado de interação durante a pausa
...
function Movement.set_interaction_end_time(end_time)
    interaction_end_time = end_time + INTERACTION_DURATION  -- Adiciona duração extra à interação
    state = States.INTERACTING
    Movement.set_moving(false)
    Movement.set_explorer_control(false)
    Movement.disable_anti_stuck()
