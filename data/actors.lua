...
function actors.update()
    ...
    -- Inicializar ouro inicial se ainda não foi feito
...
function actors.draw_actor_info()
    ...
    -- Desenha as informações dos atores
    ...
    -- Adiciona a exibição do ouro
    local gold_text = string.format("Total Gold Gained: %s", format_number(gold_gained))
    graphics.text_2d(gold_text, vec2:new(positions.gold.x, positions.gold.y), 20, color_white(255))
end
